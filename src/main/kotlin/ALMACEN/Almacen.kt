...
class Almacen(almacen_mapa:MutableMap<Int, MutableList<Bebida>>) {//fin clase
    ...
    fun agregar2(bebida_nueva: Bebida){
        ...
        //+ elegante coger el tamaño de la primera estantería antes de entrar al mapa
        //var posicion=estanterias.keys.min()
        //var tamanio=estanterias[estanterias.keys.min()]?.size
        //en tamanio en el if habría que poner tamanio !!
        //la doble exclamación !! es por si intllij encuentra un nulo
    ...
    fun agregar3(bebida_nueva: Bebida){
        ...
        for ((num_estanteria, estanteria) in estanterias){
            ...
            if (contador>=contador_almacen){ //mayor o igual porque si no hay bebida de esa marca, se debe agregar
                //a alguna estantería igualmente
    ...
    fun eliminar_marca(marca_dada:String){
        for (estanteria in estanterias.values){
            estanteria.removeIf{ it.marca==marca_dada }
        }}
